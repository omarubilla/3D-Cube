...
def ProjectPoints(points3d, new_intrinsics, R, T):
    ...
    for spacePos in points3d:
    	localPos = np.matmul(R, spacePos)
    	localPos += T
    	uvPos = np.matmul(new_intrinsics, localPos)
    	points2d.append([uvPos[0]/localPos[2], uvPos[1]/localPos[2]])
...
keypoint_visualization = cv2.drawKeypoints(
        reference,reference_keypoints,outImage=np.array([]), flags = cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
